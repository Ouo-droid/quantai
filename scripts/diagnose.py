...
from signals.factors.value_quality_vol import ValueFactor, VolatilityFactor
...
def test_quantagent_to_kline():
    ...
    {k: len(v) for k, v in kline.items()}
...
def test_quantagent_parse_short():
    ...
    raw = json.dumps({"decision": "SHORT", "justification": "bearish", "risk_reward_ratio": 1.4})
    direction, bias, conf = QuantAgentAdapter._parse_decision(raw)
...
def test_live_decide():
    ...
    if not api_key:
        ...
        return "[SKIP] clé absente → crée .env à partir de .env.example"
