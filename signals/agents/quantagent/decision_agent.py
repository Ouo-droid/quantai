...
def create_final_trade_decider(llm):
    ...
    def trade_decision_node(state) -> dict:
        ...
        prompt = f"""You are a high-frequency quantitative trading (HFT) analyst operating on the current {time_frame} K-line chart for {stock_name}. Your task is to issue an **immediate execution order**: **LONG** or **SHORT**. ⚠️ HOLD is prohibited due to HFT constraints.
            ...
            Base your decision on the combined strength, alignment, and timing of the following four reports:
            ...
            ### 4. GitHub Research Report:
            - Review any relevant open-source trading strategies, risk models, or backtesting frameworks discovered on GitHub.
            - If a well-starred repository implements a strategy that aligns with the current signals, factor that into your confidence.
            - Use the research context to **validate or challenge** your technical analysis.
            - Do NOT override confirmed technical signals solely based on repository findings.
            ...
            2. Prioritize decisions where **all four reports** (Indicator, Pattern, Trend, and GitHub Research) **align in the same direction**.
