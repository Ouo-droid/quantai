...
from data.client import OpenBBClient, NewsItem, FundamentalSnapshot
...
MOCK_OHLCV_RESPONSE = {
    "results": [
        ...
        {"date": "2024-01-03", "open": 186.0, "high": 188.0,
         "low": 185.0, "close": 187.5, "volume": 45_000_000},
        {"date": "2024-01-04", "open": 187.0, "high": 189.0,
         "low": 186.0, "close": 185.0, "volume": 60_000_000},
    ]
}
...
class TestNews:
    def test_news_returns_list_of_newsitem(self, mock_client):
        ...
        mock_resp.raise_for_status = MagicMock()
        mock_resp.json.return_value = {"results": [
            {"date": "2024-01-01T12:00:00", "title": "Apple hits ATH",
             "text": "Apple stock...", "url": "https://example.com", "source": "Reuters"},
        ]}
        mock_client._client.get.return_value = mock_resp
